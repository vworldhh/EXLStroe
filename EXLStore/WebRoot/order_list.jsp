		...
		$(function(){
			$(".del").click(function(){
				if(confirm("您确定删除该订单吗？")){
					...
					location.href="${ app }/servlet/OrderDeleteServlet?oid="+oid;
				}
			});
		});
	</script>
...
<c:if test="${empty list}">
		  <center> <h2> <span style="color:red;"> 你当前的订单为空！！ </span></h2> </center>
</c:if>
	<!-- 模版数据 -start -->
			...
			<dd style="line-height: 26px;">
				...
				支付状态：
					...
					<c:if test="${ orderInfo.order.paystate == 1 }">
						...
						<a href="javascript:void(0)" class="del" id="${ orderInfo.order.id }"><!-- 订单删除 -->
							<img src="${ app }/img/orderList/sc.jpg" width="69" height="19"/>
						</a>
						&nbsp;
					 <c:if test="${ orderInfo.order.paystate == 0 }">
				 		<a href="${ app }/pay.jsp?oid=${ orderInfo.order.id }&money=${ orderInfo.order.money }"> <!-- 在线支付 -->
